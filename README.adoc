...
> 기능과 구조 사이에선 구조가 중요하다. 구조를 통해 ‘나름의’ 깔끔한 규칙을 유지하는 것을 통해 유지보수를 위한 맨먼스를 줄일 수 있기 때문이다.
...
이를 통해 Data Access(Infrastruture)를 변경해도 내부 Application Core에는 전혀 수정을 가하지 않는 유지보수를 가능케 하는 구조를 만들 수 있었습니다. @UseCase, @Infrastrcuture 어노테이션을 사용해 Application Service Layer와 Infrastructure Service Layer를 완벽하게 분리하여, 전통적인 Layered Architecture에서 겪을 수 있는 Service Layer의 @Service 어노테이션이 의미도 명시적으로 표현했습니다.
